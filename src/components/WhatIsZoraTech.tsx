...
const icons = [
  <svg key="1" width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
  </svg>,
  <svg key="2" width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
  </svg>,
  <svg key="3" width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
function PillarCard({ icon, title, description, index }: { icon: React.ReactNode; title: string; description: string; index: number }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-zt-text font-semibold text-lg mb-3 [font-family:'Playfair_Display',serif]">{title}</h3>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-2xl md:text-3xl text-zt-text/80 max-w-3xl mx-auto leading-relaxed [font-family:'Playfair_Display',serif]">
            {t("desc")}{" "}
            <span className="text-zt-gold">{t("descHighlight")}</span>{" "}
            {t("descEnd")}
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {pillars.map((pillar, i) => (
            <PillarCard key={i} icon={icons[i]} title={pillar.title} description={pillar.desc} index={i} />
          ))}
        </div>
      </div>
    </section>
